...
def logis c_regression(X, y, num_itera ons=200, learning_rate=0.001): 
    weights = np.zeros(X.shape[1]) 
    for _ in range(num_itera ons): 
        z = np.dot(X, weights) 
        h = sigmoid(z) 
...
y = (iris.target != 0) * 1  # Convert to binary classifica on 
# Split the dataset 
...
# Perform logis c regression 
weights = logis c_regression(X_train_std, y_train) 
# Make predic ons 
...
plt.sca er(X_train_std[:, 0], X_train_std[:, 1], c=y_train, alpha=0.8) 
