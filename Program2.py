# The minimax func on is the heart of the AI. It recursively calculates the opmal move for the AI. 
def minimax(total, turn, alpha, beta): 
    ...
    if turn: 
        max_eval = -float('inf')  # Ini alize max_eval to nega ve infinity 
        for i in range(1, 4):  # For each possible move (1, 2, or 3) 
            ...
            if beta <= alpha:  # If beta is less than or equal to alpha, break the loop (alpha-beta pruning) 
                break 
        return max_eval  # Return the maximum evalua on 
    # If it's the human's turn, we want to minimize the score 
    else: 
        min_eval = float('inf')  # Ini alize min_eval to posi ve infinity 
        for i in range(1, 4):  # For each possible move (1, 2, or 3) 
            # Recursively call minimax for the next state of the game 
            ...
            if beta <= alpha:  # If beta is less than or equal to alpha, break the loop (alpha-beta pruning) 
                break 
        return min_eval  # Return the minimum evalua on 
 
# The total score of the game is ini ally 0 
total = 0 
...
while True: 
    ...
    while human_move not in [1, 2, 3]:  # If the move is not valid, ask for input again 
        ...
        human_move = int(input("Enter your move (1, 2, or 3): ")) 
    total += human_move 
    print(f"A er your move, total is {total}") 
    if total >= 20:  # If the total is 20 or more a er the human's move, the human wins 
        print("You win!") 
        break 
    ...
    max_eval = -float('inf') 
    for i in range(1, 4):  # For each possible move (1, 2, or 3) 
        # Call minimax to get the evalua on of the move 
        eval = minimax(total + i, False, -float('inf'), float('inf')) 
        if eval > max_eval:  # If the evalua on is greater than max_eval, update max_eval and ai_move 
            max_eval = eval 
            ai_move = i 
    total += ai_move  # Add the AI's move to the total 
    print(f"AI adds {ai_move}. Total is {total}") 
    if total >= 20:  # If the total is 20 or more a er the AI's move, the AI wins 
        print("AI wins!") 
        break
